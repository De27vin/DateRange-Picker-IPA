...
trait EncryptsAttributesNotSerialize {
    ...
    public function attributesToArray()
    {
        ...
        foreach ($this->getEncrypts() as $key) {
            if (array_key_exists($key, $attributes)) {
                try {
                    $attributes[$key] = decrypt($attributes[$key], false);
    ...
    public function getAttributeValue($key)
    {
        if (in_array($key, $this->getEncrypts())) {
            try {
                return decrypt($this->attributes[$key], false);
            } catch (DecryptException $e) {
                // handle decryption failure
                if (method_exists($this, 'handleEncryptionError')) {
                    $this->handleEncryptionError($key, $e);
    ...
    {
        if (in_array($key, $this->getEncrypts())) {
            $this->attributes[$key] = encrypt($value, false);
        } else {
