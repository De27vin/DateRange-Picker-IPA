...
@php
    ...
    $model = $isCharts ? 'chartsSettings' : 'dashboardWidgetSettings';
    $updateAction = $isCharts ? 'updateChartsSettings' : 'updateDashboardWidgetSettings';
    $cancelAction = $isCharts ? 'cancelChartsSettings' : 'cancelDashboardWidgetSettings';
@endphp
...
<div class="pb-12 mx-auto px-4">
    <x-page.header class="mt-8 mb-2 h-20">
        ...
        <x-slot name="description">{{ $isCharts ? __('Account-wide rolling defaults for the Charts page date ranges.') : __('Account-wide defaults for the dashboard cards. Users can still change their own dashboard view and reset it back to these defaults.') }}</x-slot>
                        ...
                        <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_8rem_10rem] md:items-end">
                            ...
                            <label class="grid gap-1 text-sm text-gray-600">
                                ...
                                <input
                                    ...
                                    wire:model.defer="{{ $model }}.ranges.{{ $item }}.amount"
                                >
                            </label>
                            ...
                            <label class="grid gap-1 text-sm text-gray-600">
                                ...
                                <select
                                    class="rounded border-gray-300"
                                    wire:model.defer="{{ $model }}.ranges.{{ $item }}.unit"
                                >
