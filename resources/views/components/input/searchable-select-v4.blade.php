...
>
    ...
    <div
            @click="toggle()"
            :class="{ 'border-color-new-300': open }"
            class="form-select block w-full h-16 pl-3 pr-3 pt-8 mt-0 mb-1 text-normal leading-6 border border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-color-new-300 transition-colors duration-200 {{ $trailingAddOn ? ' rounded-r-none' : '' }} {{ $disabled ? 'bg-gray-100 cursor-not-allowed' : 'cursor-pointer' }}"
    >
        <div class="truncate flex justify-between items-center">
            <span x-text="selectedText || '{{ $placeholder }}'" class="truncate text-gray-700"></span>
            <div class="flex items-center absolute" style="right: 17px;top: 23px;">
                <button
                ...
                >
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="position: absolute; right: 36px; top: -4px;">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="position: absolute; right: 1px; top: 1px;">
    ...
    >
        <div class="p-2 sticky top-0 bg-white border-b border-gray-200">
            <input
                    ...
                    x-model="search"
                    type="text"
                    class="h-10 w-full px-3 border border-gray-300 text-normal"
                    placeholder="Search..."
        ...
        <template x-for="option in [...$refs.select.options].slice(1).filter(option => option.textContent.toLowerCase().includes(search.toLowerCase()))" :key="option.value">
            <div
                    ...
                    class="px-3 py-2 hover:bg-gray-100 cursor-pointer text-normal text-gray-700"
                    :class="{ 'bg-gray-50': option.value === selected }"
        ...
        <div
                x-show="[...$refs.select.options].slice(1).filter(option => option.textContent.toLowerCase().includes(search.toLowerCase())).length === 0"
