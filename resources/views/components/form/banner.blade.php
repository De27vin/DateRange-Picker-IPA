...
>
	<div
		{{ 
			$attributes->class([
				'bg-'.$color.'-600 text-gray-50 dark:bg-'.$color.'-600 dark:text-gray-100 bg-opacity-40 font-medium flex items-center px-2 py-1',
	...
	>
		...
		@if($icon)
			<div class="shrink-0 mr-1">
				<x-form.icon :icon="$icon" color="white" size="sm" />
			</div>
		@endif
			...
			<div class="flex-shrink-0 pointer-events-auto">
				<button type="button"
					...
					{{ $attributes->class([
						...
						'hover:bg-color-new focus:bg-color-new' => ($color == "primary"),
