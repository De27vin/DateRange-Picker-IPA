...
<span x-cloak
    ...
    <button
        ...
        @if($icon === 'sort')
            <span class="block h-8 w-8 hover:text-color-new-600">
        ...
        @elseif(!empty($icon))
            <span class="tt">
                <i class="f7-icons icon icon-sm tts cursor-pointer" style="color: white; background-color: #8faadc;">{{ $icon }}</i>
                <span class="ttt elip ttt-tl bg-white border border-slate-300 text-dark shadow-md text-sm" style="width: max-content; zoom: 1.2; text-transform: none; color: #334155;">{{ __('Exports') }}</span>
            </span>
        @else
            <span class="block w-8 h-8 ml-4 hover:text-color-new-600">
                <svg class="block h-full" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10 12a2 2 0 1 0 4 0 2 2 0 0 0-4 0zm0-6a2 2 0 1 0 4 0 2 2 0 0 0-4 0zm0 12a2 2 0 1 0 4 0 2 2 0 0 0-4 0z" fill="currentColor"/></svg>
            </span>
    ...
    <div 
        ...
        class="z-50 origin-top-right absolute right-0 h-full-mr-1 w-auto shadow-lg focus:outline-none" 
        role="menu" 
        tabindex="-1" 
        ...
        <div class="py-1 bg-white overflow-hidden dropdown" role="none">
            {{ $slot }}
        </div>
    </div>
</span>
