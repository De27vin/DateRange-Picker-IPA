<template>
  <div class="test">
    <div v-if="loading" class="w-full text-center" style="margin-block: 5rem;">
      <pulse-loader :loading="loading" :color="colors.default" :size="'2rem'"></pulse-loader>
    </div>

    <div class="site-box" v-if="!!site">

      <!--NORMAL SITE-->
      <component class="selectable-link" :is="!isEmpty(deviceModeId) && !actionsForbidden.includes('goToSite') ? 'a' : 'div'" :href="!isEmpty(deviceModeId) && !actionsForbidden.includes('goToSite') ? '/device-site/'+site.ds_id : ''" draggable="false" @click.prevent>
      <div class="site-row selectable-content" :style="{'height': (activeLabels && site.labels && site.labels.length > 0) ? '8rem' : '5rem' }" v-if="!site.edited" @click="!isEmpty(deviceModeId) && !actionsForbidden.includes('goToSite') && navigateToSite(site)">
...
<!--              <a v-if="!isEmpty(deviceModeId)" :href="'/device-site/'+site.ds_id">-->
<!--                <span class="icon-wrapper tt">-->
...
<!--            </div>-->
      ...
      <div v-for="device in site.devices" :key="device.device_id">
        ...
        <!--NORMAL DEVICE ROW-->
        <!--        THIS IS RESTRICTING ENTERING DEVICE DETAILS-->
        <!--        <component class="selectable-link" :is="isEmpty(deviceModeId) && !actionsForbidden.includes('goToDevice') ? 'a' : 'div'" :href="isEmpty(deviceModeId) && !actionsForbidden.includes('goToDevice') ? '/devices/'+device.device_id : ''" draggable="false" @click.prevent>-->
        <component class="selectable-link" :is="'div'" :href="''" draggable="false" @click.prevent>
        <div class="device-row selectable-content flex justify-between items-center" v-if="!device.edited" @click="false">
          ...
          <div class="flex items-start" style="width: 27%">
            ...
            <span class="icon-wrapper tt" v-if="device.gateway">
              <a :href="(!isEmpty(device.gatewayLink) && !actionsForbidden.includes('gatewayLink')) ? device.gatewayLink : '#'"
                  @click.stop="(isEmpty(device.gatewayLink) || actionsForbidden.includes('gatewayLink')) ? $event.preventDefault() : ''"
                  :style="{ cursor: (isEmpty(device.gatewayLink) || actionsForbidden.includes('gatewayLink')) ? 'default' : 'pointer' }"
              >
            ...
            <!--callcenter alarming indicator-->
            <span class="icon-wrapper tt" v-if="shouldShowCallcenterAlarmIndicator(device) && !device.isActiveAlarm">
              <i @click.prevent.stop class="f7-icons icon icon-sm tts cursor-default" :style="{ 'background-color': colors.alarm }">bell</i>
              <span @click.prevent.stop class="ttt elip ttt-br bg-white border border-slate-300 text-dark shadow-md text-sm" style="width: max-content; zoom: 1.2;">{{ trans('Alarm Call') }}</span>
            </span>
          </div>
          ...
          <div class="gr-6" style="width: 150%">
            ...
            <div class="gs-1 flex items-center">
              <span class="icon-wrapper tt">
                <i @click.prevent.stop class="f7-icons icon default icon-sm tts cursor-default" style="background-color: lightblue">tag</i>
                <span @click.prevent.stop class="ttt elip ttt-tr bg-white border border-slate-300 text-dark shadow-md text-sm" style="width: max-content; zoom: 1.2;">{{ `${trans('Equipment')} (ID: ${device.device_id})` }}</span>
              </span>
              <span class="tt-vue">
          ...
          <div class="gr-1 flex flex-col items-start gap-2 mr-4" style="width: 20%">
            <div class="flex justify-end" v-if="!device.edited">
              ...
              <span class="icon-wrapper tt callcenter-phone-wrapper"
                    v-if="shouldShowAlarmPhoneButton(device)"
                    @click.prevent.stop="isAlarmPhoneClickable(device) && handleAlarmPhoneClick(device)">
                <i class="f7-icons icon default icon-sm tts"
                   :class="isAlarmPhoneClickable(device) ? 'cursor-pointer' : 'cursor-not-allowed'"
                   :style="getAlarmPhoneButtonStyle(device)">phone
                </i>
                <span class="ttt elip ttt-tl bg-white border border-slate-300 text-dark shadow-md text-sm"
                      style="width: max-content; zoom: 1.2;">{{ trans('Alarm Call') }}</span>

                <transition name="fade">
                  <div class="callcenter-classification-dropdown" v-if="showClassificationDropdown" @click.stop>
                    <button type="button"
                            v-for="classification in classificationOptions"
                            :key="classification.key"
                            class="callcenter-classification-option"
                            :style="{ borderLeftColor: classification.color }"
                            @click.prevent="classifyAlarmCall(classification)">
                      <span class="option-icon" :style="{ color: classification.color }">
                        <i class="f7-icons">{{ classification.icon }}</i>
                      </span>
                      <span class="option-label">{{ classification.label }}</span>
                      <span class="option-shortcut">{{ classification.fkey }}</span>
                    </button>
                  </div>
                </transition>
              </span>

              <span class="icon-wrapper tt" v-else-if="shouldShowCarCallButton(device) && device.actionButtons.carcall && !actionsForbidden.includes('carcall')" @click.prevent.stop="makeFsCall('carcall', device)">
                <i class="f7-icons icon default icon-sm tts cursor-pointer"
              ...
              <span class="icon-wrapper tt" v-if="device.actionButtons.trigger && !actionsForbidden.includes('trigger')" @click.prevent.stop="makeFsCall('trigger', device)">
                <span v-if="bigBtns" class="tts cursor-pointer p-2"
                      style="padding: 0.5rem; letter-spacing: 0.1rem; text-transform: uppercase;"
                      :style="{'background-color': device.actionButtons.trigger === 'progress' ? 'greenyellow' : '#8faadc', 'color': device.actionButtons.trigger === 'progress' ? 'black' : 'white'}">
                  {{ trans('Trigger') }}
                </span>
                <i v-if="!bigBtns" class="f7-icons icon default icon-sm tts cursor-pointer"
              ...
              <span class="icon-wrapper tt" v-if="device.actionButtons.revival && !actionsForbidden.includes('revival')" @click.prevent.stop="makeFsCall('revival', device)">
                <i class="f7-icons icon default icon-sm tts cursor-pointer"
              ...
              <span class="icon-wrapper tt" v-if="device.actionButtons.set && !actionsForbidden.includes('set')" @click.prevent.stop="makeFsCall('set', device)">
                <i class="f7-icons icon default icon-sm tts cursor-pointer"
              ...
              <span class="icon-wrapper tt" v-if="device.actionButtons.mute && !actionsForbidden.includes('mute')" @click.prevent.stop="makeFsCall('mute', device)">
                <i class="f7-icons icon default icon-sm tts cursor-pointer"
...
<!--              <a v-if="isEmpty(deviceModeId) && !actionsForbidden.includes('goToDevice')" :href="'/devices/'+device.device_id">-->
<!--                <span class="icon-wrapper tt">-->
<!--                  <i class="f7-icons icon default icon-sm tts cursor-pointer"-->
<!--                     style="background-color: #8faadc;-->
<!--                     color: white;">arrow_up_right-->
...
<!--    THIS IS RESTRICTING DEVICE DETAILS ACCESS-->
<!--    <component class="selectable-link" :is="isEmpty(deviceModeId) && !actionsForbidden.includes('goToDevice') ? 'a' : 'div'" :href="isEmpty(deviceModeId) && !actionsForbidden.includes('goToDevice') ? '/devices/'+device.device_id : ''" draggable="false" @click.prevent>-->
        <component class="selectable-link" :is="'div'" :href="''" draggable="false" @click.prevent>
        <div class="device-last-row selectable-content flex justify-between items-center" v-if="!device.edited && (!isEmpty(device.alarmNumber) || !isEmpty(device.periodicalNumber))" @click="false">
...
<!--            </div>-->
      ...
      <div v-if="!site.edited && site.showComments">
        ...
        <div class="flex justify-between items-start" :class="{'ml-4': !isEmpty(deviceModeId)}" style="min-height: 2rem; margin-block: 1.3rem;">

          <div v-if="isEmpty(deviceModeId)" class="flex items-start justify-start ml-4" style="width: 27%;">
            <div style="width: 100%;">
          ...
          <div :class="isEmpty(deviceModeId) ? 'gr-9' : 'gr-10'" style="width: 170%">
            <div :class="isEmpty(deviceModeId) ? 'gs-7' : 'gs-8'">
        ...
        <div v-if="site.comments.length">
          ...
          <div class="flex justify-between items-center h-14">
            <div class="uppercase ml-4 mt-4 -mb-2">{{ isEmpty(deviceModeId) ? trans('site comments') : trans('device comments') }}</div>
          </div>
...
<script>
  ...
  import CallcenterModeMixin from "./mixins/CallcenterModeMixin";
  ...
  export default {
    ...
    mixins: [NotificationsMixin, LabelsMixin, CallcenterModeMixin, SettingsMixin],
    ...
    methods: {
      ...
      async initialFetch(siteId) {
        ...
        try {
          ...
          let [customFieldsConfigRes, siteRes, settingsRes, countriesRes, requiredRes, labelsRes, translationsRes, gatewaysRes, sipRes] = await Promise.all([
            axios.get('/data/cfg'),
            axios.post('/equipment/site', { siteId: this.siteId }),
          ...
          if (!isEmpty(this.deviceModeId)) {
            site.devices = site.devices.filter(device => device.device_id === this.deviceModeId)
          }

          this.unpackSiteData(site)
          this.site = site

          this.paginationData.total = siteRes.data.last_page
      ...
      unpackSiteData(site) {
        ...
        site.edited = false
        site.showComments = this.initiallyOpenComments || false;
        ...
        site.errors = {}
        if (!isEmpty(this.deviceModeId)) {
          site.comments = site.comments.filter(c => c.dc_device_id === this.deviceModeId);
        }

        // for basf there was some kind of issue with processing that
        if (this.activeLabels && !this.basfLink) {
      ...
      startEdit(site) {
        ...
        site.selectedSipNumber = site.sip ? {
            id: null,
            name: site.sip
        } : null;
        ...
        axios.post('/equipment/saveSite', site)
          .then(response => {
            ...
            if (response.data.success === true) {
              ...
              // only for site details
              if (!isEmpty(this.deviceModeId)) {
                savedSite.devices = savedSite.devices.filter(device => device.device_id === this.deviceModeId)
              }
        ...
        axios.post('/equipment/rejectSet', { deviceId: device.device_id, field: field })
          .then(response => {
            if (response.data.success === true) {
              ...
              this.unpackDeviceData(newDevice, this.site)
      ...
      reloadHistory(siteId, start, end) {
        ...
        axios.post('/equipment/siteHistory', { siteId: siteId, start: requestStart, end: requestEnd })
          .then(response => {
            ...
            this.$refs['timeline_'+siteId].initTimeline(historyArray, start, end)
      ...
      makeFsCall(action, device) {
        ...
        // For carcall, start 20-second timeout
        if (action === 'carcall') {
        ...
        axios.post('/equipment/fsCall', { action: action, deviceId: device.device_id })
          ...
          .finally(() => {
            window.dispatchEvent(new CustomEvent('loading', { detail: { action: 'equipment', loading: false }} ));
            this.actionInProgress = false
            // For carcall, don't reset - wait for START/END events or timeout
            if (action !== 'carcall') {
      ...
      handleCarCallStatusEvent(event) {
        ...
        const device = this.site?.devices?.find(dev => String(dev.device_id) === deviceKey);
        if (!device || !device.actionButtons?.carcall) {
          console.log('device or device carcall not found - returning')
          return;
        }
        ...
        if (status === 'start') {
          this.$set(device.actionButtons, 'carcall', 'active');
        } else if (status === 'end') {
          this.$set(device.actionButtons, 'carcall', true);
        }
      ...
      addComment(site) {
        ...
        if (isEmpty(this.deviceModeId) && isEmpty(site.newCommentEquipment)) {
        ...
        window.dispatchEvent(new CustomEvent('loading', { detail: { action: 'equipment', loading: true }}))
        axios.post('/equipment/addComment', { deviceId: site.newCommentEquipment || this.deviceModeId, newComment: site.newCommentContent, link: site.newCommentLink })
          .then(response => {
            if (response.data.success === true && response.data.comments) {
              ...
              // rerender comments
              if (!isEmpty(this.deviceModeId)) {
                site.comments = response.data.comments.filter(c => c.dc_device_id === this.deviceModeId);
              } else {
      ...
      openCliConfirmationModal(site) {
        let newNumber = site.cloned.sip || site.cloned.sim || site.cloned.pbx || site.cloned.pstn || ''
        window.dispatchEvent(new CustomEvent('openModal_cliConfirmationModal', { detail: { newNumber: newNumber, cliNumber: site.cliNumber, cliNumberLevel: site.cliNumberLevel || this.trans('not entered') }} ))
      },
    ...
    created() {
      console.log('VueEquipmentSite created() hook called', {
        siteId: this.siteId,
        deviceModeId: this.deviceModeId,
        callcenterMode: this.callcenterMode
      });
      ...
      this.initialFetch(this.siteId)

      const self = this;
      window.addEventListener('siteUpdated', (event) => {
        if (self.site.ds_id === event.detail.siteId) {
          self.reloadSite(self.site);
        }
      })
      ...
      window.addEventListener('refreshSiteTile', () => {
        console.log('received refreshSiteTile')
        if (self.site) {
          self.reloadSite(self.site);
        }
      });
    ...
    mounted() {
      console.log('VueEquipmentSite mounted() hook called', {
        siteId: this.siteId,
        deviceModeId: this.deviceModeId,
        callcenterMode: this.callcenterMode,
        site: this.site
    ...
    beforeDestroy() {
      console.log('VueEquipmentSite beforeDestroy() hook called');
...
<style lang="scss" scoped>
  ...
  .callcenter-phone-wrapper {
    position: relative;
  }

  .callcenter-classification-dropdown {
    position: absolute;
    top: 110%;
    right: 0;
  ...
  .callcenter-classification-option {
    display: flex;
    align-items: center;
    width: 100%;
  ...
  .callcenter-classification-option:last-child {
    border-bottom: none;
  }

  .callcenter-classification-option:hover {
    background-color: #f9fafb;
  }

  .callcenter-classification-option .option-icon {
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 1.25rem;
  ...
  .callcenter-classification-option .option-label {
    flex: 1;
    font-weight: 400;
    line-height: 1.4;
  }

  .callcenter-classification-option .option-shortcut {
    font-weight: 600;
    color: #9ca3af;
    font-size: 0.75rem;
